...
\begin{document}
	...
	\begin{itemize}
	\item \textbf{symulator przestrzeni roboczej}:
	
	odpowiedzialny za wizualizację oraz fizyczne oddziaływania pomiędzy elementami (np. kolizje), a także za wyznaczanie odczytów czujników (oprócz czujników skażenia);
	
	\item \textbf{symulator robotów}:
	
	odpowiedzialny za wyznaczanie sterowania robotów na podstawie odczytów z czujników, realizujący określone zadanie;
	
	\item \textbf{symulator chmury skażenia}:
	
	odpowiedzialny za generowanie odczytów czujników skażenia;
